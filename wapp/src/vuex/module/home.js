...
const home = {
  ...
  mutations: {
    ...
    hotClick(state,index){//热门作者存储
      window.localStorage.removeItem("itemObj");
      let authorData = state.author[index];
      console.log(JSON.stringify(authorData));
      let itemStr = JSON.stringify(authorData);
      ...
      get("/aritcle", prams).then(res => {
        if (res.status == "success" && res.code == 200) {
          ...
          for(var i=0; i<moreData.length;i++){
            if(moreData[i].author_pic == "" || moreData[i].author_pic == false){
              moreData[i].author_pic = "/static/images/icon/author.svg"
            }else{
              moreData[i].author_pic = moreData[i].author_pic
            }
    ...
    navTags(state,res){
      if (res.status == "success" && res.code == 200) {
        state.tabsData =res.data.slice(0,5);
